...
#
#
# def test_end_to_end_live_mse(integration_scanner, end_to_end_manga_name):
#     """This test is designed for triggering a live test in a local environment"""
#     if end_to_end_manga_name is None:
#         return
#
...
def test_end_to_end_live_cmk(integration_scanner, end_to_end_manga_name):
    """This test is designed for triggering a live test in a local environment"""
    if end_to_end_manga_name is None:
        return

    live_db = ChapterEntityDB()
    live_db.update([end_to_end_manga_name])

    query_params = {"ids[]": [end_to_end_manga_name]}
    chapters = plugins["cmk"].from_server_url(query_params)
    assert len(chapters) > 1
    for idx, chapter in enumerate(chapters):
        if idx > 0:
            break
