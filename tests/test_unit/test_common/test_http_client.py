...
from cbz_tagger.common.http_client import get_request_configs
...
def test_request_configs_user_agents_match_declared_browser_and_are_not_flagged():
    ...
    for config in get_request_configs():
...
@patch("cbz_tagger.common.http_client.random.uniform", return_value=1.0)
...
def test_request_with_retry_rotates_through_configs(mock_sleep, mock_random):
    ...
    with requests_mock.Mocker() as rm:
        ...
        with pytest.raises(EnvironmentError):
            request_with_retry("http://example.com/file", retries=len(configs))
...
def test_request_with_retry_success(mock_sleep, mock_random):
    with requests_mock.Mocker() as rm:
        ...
        result = request_with_retry("http://example.com/file")
...
def test_request_with_retry_timeout(mock_sleep, mock_random):
    with requests_mock.Mocker() as rm:
        ...
        with pytest.raises(
            ...
            request_with_retry("http://example.com/file")
...
@patch("cbz_tagger.common.http_client.random.uniform", return_value=1.0)
@patch("cbz_tagger.common.http_client.time.sleep")
@patch("cbz_tagger.common.http_client.AppEnv")
