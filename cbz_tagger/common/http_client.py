...
This is the single place where the application talks to the network. It was previously
~180 lines living on BaseEntity, which meant every domain model — AuthorEntity has ten
lines of logic — inherited an anti-bot user-agent rotation stack.
"""
...
def get_request_configs() -> list[dict]:
...
def download_file(url):
    return request_with_retry(url).content


def unpaginate_request(url, query_params=None, limit=100) -> list[dict[str, Any]]:
